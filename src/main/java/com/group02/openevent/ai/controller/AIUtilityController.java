...
public class AIUtilityController {
    ...
    public ResponseEntity<Map<String, Object>> classifyIntent(@RequestBody Map<String, String> request) {
        try {
            ...
            ActionType intent = intentClassifier.classifyIntent(text);
            String weatherIntent = intentClassifier.classifyWeather(text);
            String toolIntent = intentClassifier.classifyToolEvent(text);
    ...
    public ResponseEntity<Map<String, Object>> checkHealth() {
        try {
            ...
            try {
                float[] testEmbedding = embeddingService.getEmbedding("test");
                health.put("embeddingService", Map.of("status", "UP", "vectorSize", testEmbedding.length));
            } catch (Exception e) {
            ...
            try {
                ActionType testIntent = intentClassifier.classifyIntent("test");
            ...
            // Kiểm tra QdrantService
            try {
                List<Map<String, Object>> testSearch = qdrantService.searchSimilarVectors(new float[]{0.1f}, 1);
                health.put("qdrantService", Map.of("status", "UP", "searchResult", testSearch.size()));
            } catch (Exception e) {
                health.put("qdrantService", Map.of("status", "DOWN", "error", e.getMessage()));
            }
