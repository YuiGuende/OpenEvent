    ...
    public Map<String, Object> debugTopStudents() {
        ...
        try {
            ...
            for (int i = 0; i < Math.min(5, allCustomers.size()); i++) {
                ...
                sample.put("name", c.getUser().getName());
                sample.put("points", c.getPoints());
                sample.put("email", c.getUser().getAccount().getEmail());
    ...
    public Map<String, Object> debugTopStudentsRaw() {
        ...
        try {
            ...
            for (Customer c : allCustomers) {
                if (c.getUser().getName() == null || c.getUser().getName().trim().isEmpty()) {
                    Map<String, Object> detail = new HashMap<>();
                    detail.put("id", c.getCustomerId());
                    detail.put("name", c.getUser().getName());
                    detail.put("points", c.getPoints());
                    detail.put("email", c.getUser().getAccount().getEmail());
