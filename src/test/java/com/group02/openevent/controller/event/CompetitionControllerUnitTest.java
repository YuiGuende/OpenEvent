...
@DisplayName("CompetitionController Integration Tests (Full Coverage)")
class CompetitionControllerIntegrationTest {
    ...
    @Nested
    @DisplayName("1️⃣ Competition Event Detail Tests")
    class CompetitionEventDetailTests {

        @Test
        @DisplayName("COMPETITION-001: Khi tìm thấy event, trả về view với eventDetail (200 OK)")
        void whenEventFound_thenReturnViewWithEventDetail() throws Exception {
            Long eventId = 1L;
            CompetitionEventDetailDTO eventDetail = new CompetitionEventDetailDTO();

            when(competitionService.getCompetitionEventById(eventId)).thenReturn(eventDetail);

            mockMvc.perform(get("/competition/{id}", eventId))
                    .andDo(print())
                    .andExpect(status().isOk())
                    .andExpect(view().name("competition/competitionHome"))
        ...
        @Test
        @DisplayName("COMPETITION-002: Khi không tìm thấy event, service throw Exception, trả về view với error")
        void whenEventNotFound_thenReturnViewWithError() throws Exception {
            Long eventId = 999L;

            when(competitionService.getCompetitionEventById(eventId))
            ...
            mockMvc.perform(get("/competition/{id}", eventId))
                    ...
                    .andExpect(model().attribute("error", "Không thể tìm thấy cuộc thi bạn yêu cầu."));
        }

        @Test
        @DisplayName("COMPETITION-003: Khi service throw IllegalArgumentException, trả về view với error")
        ...
        @Test
        @DisplayName("COMPETITION-004: Khi service throw NullPointerException, trả về view với error")
        ...
        @Test
        @DisplayName("COMPETITION-005: Khi eventDetail có đầy đủ thông tin, model chứa tất cả data")
        ...
        @Test
        @DisplayName("COMPETITION-006: Khi eventId là 0, service throw exception, trả về error")
        ...
        @Test
        @DisplayName("COMPETITION-007: Khi eventId là số âm, service throw exception, trả về error")
